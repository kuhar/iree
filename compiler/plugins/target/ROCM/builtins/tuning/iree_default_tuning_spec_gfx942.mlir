...
transform.named_sequence @__kernel_config(%variant_op: !transform.any_op {transform.readonly}) -> ()
  ...
  %mmt, %config = transform.include @match_mmt_2048x1280x5120_f16_f16_f32 failures(suppress) (%variant_op)
    : (!transform.any_op) -> (!transform.any_op, !transform.any_param)
  transform.include @apply_op_config failures(propagate) (%variant_op, %config)
    : (!transform.any_op, !transform.any_param) -> ()
  transform.yield
}
