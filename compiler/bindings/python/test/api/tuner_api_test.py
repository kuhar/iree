...
@run
def test_materialize_compilation_info_from_constraints():
    module_str = """
        module {
            iree_codegen.smt.constraints
                target = <set = 0>,
                ...
                knobs = {
                    ...
                    subgroup_basis = [[#iree_codegen.smt.int_knob<"sg_m_cnt">,
                                       #iree_codegen.smt.int_knob<"sg_n_cnt">,
                                       1],
                                      [0, 1, 2]],
                    workgroup_size = [#iree_codegen.smt.int_knob<"wg_size_x">,
                                      #iree_codegen.smt.int_knob<"wg_size_y">,
                                      #iree_codegen.smt.int_knob<"wg_size_z">],
    ...
    assert str(compilation_info.lowering_config) == (
        ...
        "#iree_gpu.mma_layout<MFMA_F32_32x32x8_F16>, "
        "reduction = [0, 0, 8], subgroup_basis = [[2, 1, 1], [0, 1, 2]], "
        "workgroup = [64, 128, 0]}>"
    )
    ...
    try:
        ...
        assert False, "expected missing mma_idx assignment to fail"
    except RuntimeError as e:
        assert "materialization" in str(e)
