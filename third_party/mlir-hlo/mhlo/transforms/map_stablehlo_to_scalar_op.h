...
#ifndef MLIR_HLO_StableHLO_TRANSFORMS_MAP_STABLEHLO_TO_SCALAR_OP_H
#define MLIR_HLO_StableHLO_TRANSFORMS_MAP_STABLEHLO_TO_SCALAR_OP_H
...
// A struct to map StableHloBinaryOpTy type to the corresponding floating-point and
...
// Alias for the map from StableHLO binary op type to STD floating-point op type.
template <typename StableHloOp>
using ScalarFOp = typename StableHloToScalarOp<StableHloOp>::FOp;
// Alias for the map from StableHLO binary op type to STD signed integer op type.
template <typename StableHloOp>
using ScalarIOp = typename StableHloToScalarOp<StableHloOp>::IOp;
// Alias for the map from StableHLO binary op type to STD unsigned integer op type.
template <typename StableHloOp>
...
struct MapStableHloOpToScalarOpImpl<SupportedType, StdScalarOp, Args...> {
  Value operator()(Location loc, ArrayRef<Type> resultTypes,
    ...
    return MapStableHloOpToScalarOpImpl<Args...>{}(loc, resultTypes, argTypes, args,
                                              b);
  }
};
...
inline Value mapStableHloOpToStdScalarOp(Location loc, ArrayRef<Type> resultTypes,
...
template <>
inline Value mapStableHloOpToStdScalarOp<stablehlo::AbsOp>(Location loc,
                                                 ArrayRef<Type> resultTypes,
                                                 ArrayRef<Type> argTypes,
                                                 stablehlo::AbsOp::Adaptor adaptor,
                                                 OpBuilder* b) {
  Type elementType = getElementTypeOrSelf(argTypes.front());
  ...
  if (elementType.isa<ComplexType>()) {
    return MapStableHloOpToScalarOpImpl<IsComplexType, ::mlir::complex::AbsOp>{}(
        loc, resultTypes, argTypes, adaptor.getOperands(), b);
  }
  if (elementType.isSignlessInteger() || elementType.isSignedInteger()) {
    ...
    auto negVal = b->create<ScalarIOp<stablehlo::SubtractOp>>(loc, zeroIntval, lhs);
...
inline std::optional<PredicateType> getCmpPredicate(stablehlo::ComparisonDirection,
                                                    bool) {
  return std::nullopt;
}
...
inline Value cmpComplex(Location loc, Value lhs, Value rhs,
                        stablehlo::ComparisonDirection comparisonDirection, OpBuilder* b) {
...
inline Value mapStableHloOpToStdScalarOp<stablehlo::CompareOp>(
  ...
  stablehlo::ComparisonDirection comparisonDirection = adaptor.getComparisonDirection();
  ...
  if (elementType.isa<IntegerType>()) {
    ...
    assert(predicate.has_value() && "expected valid comparison direction");
    return b->create<ScalarIOp<stablehlo::CompareOp>>(loc, predicate.value(), lhs,
                                                 rhs);
  }
  if (auto floatType = elementType.dyn_cast<FloatType>()) {
    ...
    std::optional<arith::CmpFPredicate> predicate =
        getCmpPredicate<arith::CmpFPredicate>(comparisonDirection,
                                              /*is_signed=*/true);
    assert(predicate.has_value() && "expected valid comparison direction");
    return b->create<ScalarFOp<stablehlo::CompareOp>>(loc, predicate.value(), lhs,
                                                 rhs);
  }
...
template <>
inline Value mapStableHloOpToStdScalarOp<stablehlo::MaxOp>(Location loc,
                                                 ArrayRef<Type> resultTypes,
                                                 ArrayRef<Type> argTypes,
                                                 stablehlo::MaxOp::Adaptor adaptor,
                                                 OpBuilder* b) {
  ...
  if (!complexTy.isa<ComplexType>())
    return MapStableHloOpToScalarOpImpl<IsFloatType, arith::MaxFOp,
...
template <>
inline Value mapStableHloOpToStdScalarOp<stablehlo::MinOp>(Location loc,
                                                 ArrayRef<Type> resultTypes,
                                                 ArrayRef<Type> argTypes,
                                                 stablehlo::MinOp::Adaptor adaptor,
                                                 OpBuilder* b) {
  ValueRange operands = adaptor.getOperands();
  Value lhs = operands.front();
  Type complexTy = lhs.getType();
  ...
  if (!complexTy.isa<ComplexType>())
    return MapStableHloOpToScalarOpImpl<IsFloatType, arith::MinFOp,
...
template <>
inline Value mapStableHloOpToStdScalarOp<stablehlo::RealOp>(Location loc,
                                                  ArrayRef<Type> resultTypes,
                                                  ArrayRef<Type> argTypes,
                                                  stablehlo::RealOp::Adaptor adaptor,
                                                  OpBuilder* b) {
  if (!adaptor.getOperand().getType().isa<ComplexType>())
    return adaptor.getOperand();
  return MapStableHloOpToScalarOpImpl<complex::ReOp>{}(loc, resultTypes, argTypes,
                                                  adaptor.getOperands(), b);
}

template <>
inline Value mapStableHloOpToStdScalarOp<stablehlo::ImagOp>(Location loc,
                                                  ArrayRef<Type> resultTypes,
                                                  ArrayRef<Type> argTypes,
                                                  stablehlo::ImagOp::Adaptor adaptor,
                                                  OpBuilder* b) {
  if (!adaptor.getOperand().getType().isa<ComplexType>())
    return b->create<arith::ConstantOp>(
        loc, b->getZeroAttr(adaptor.getOperand().getType()));
  return MapStableHloOpToScalarOpImpl<complex::ImOp>{}(loc, resultTypes, argTypes,
                                                  adaptor.getOperands(), b);
}
...
template <>
inline Value mapStableHloOpToStdScalarOp<stablehlo::DotOp>(Location loc,
                                                 ArrayRef<Type> resultTypes,
                                                 ArrayRef<Type> argTypes,
                                                 stablehlo::DotOp::Adaptor adaptor,
                                                 OpBuilder* b) {
  ...
  if (elementType.isa<IntegerType>()) {
    Value intMul =
        MapStableHloOpToScalarOpImpl<IsAnyIntegerType, ::mlir::arith::MulIOp>{}(
            loc, resultTypes, argTypes, {lhs, rhs}, b);
    return MapStableHloOpToScalarOpImpl<IsAnyIntegerType, ::mlir::arith::AddIOp>{}(
...
struct CompareSelectOpToStdScalarOp<SupportedType, StdCompareOp, Predicate,
                                    Args...> {
  static Value map(Location loc, stablehlo::ComparisonDirection comparisonDirection,
                   ArrayRef<Type> resultTypes, ArrayRef<Type> argTypes,
                   ValueRange args, OpBuilder* b) {
    Type elementType = getElementTypeOrSelf(argTypes.front());
...
template <>
inline Value mapStableHloOpToStdScalarOp<stablehlo::ClampOp>(Location loc,
                                                   ArrayRef<Type> resultTypes,
                                                   ArrayRef<Type> argTypes,
                                                   stablehlo::ClampOp::Adaptor op,
                                                   OpBuilder* b) {
...
template <>
inline Value mapStableHloOpToStdScalarOp<stablehlo::DivOp>(Location loc,
                                                 ArrayRef<Type> resultTypes,
                                                 ArrayRef<Type> argTypes,
                                                 stablehlo::DivOp::Adaptor adaptor,
                                                 OpBuilder* b) {
  Type originalType = getElementTypeOrSelf(argTypes.front());
  if (originalType.isa<ComplexType, FloatType>()) {
    return MapStableHloOpToScalarOpImpl<IsFloatType, arith::DivFOp, IsComplexType,
...
template <>
inline Value mapStableHloOpToStdScalarOp<stablehlo::RemOp>(Location loc,
                                                 ArrayRef<Type> resultTypes,
                                                 ArrayRef<Type> argTypes,
                                                 stablehlo::RemOp::Adaptor adaptor,
                                                 OpBuilder* b) {
  Type originalType = getElementTypeOrSelf(argTypes.front());
  if (originalType.isa<ComplexType, FloatType>()) {
...
template <>
inline Value mapStableHloOpToStdScalarOp<stablehlo::NegOp>(Location loc,
                                                 ArrayRef<Type> resultTypes,
                                                 ArrayRef<Type> argTypes,
                                                 stablehlo::NegOp::Adaptor adaptor,
                                                 OpBuilder* b) {
...
template <>
inline Value mapStableHloOpToStdScalarOp<stablehlo::NotOp>(Location loc,
                                                 ArrayRef<Type> /*ResultTypes*/,
                                                 ArrayRef<Type> /*argTypes*/,
                                                 stablehlo::NotOp::Adaptor adaptor,
                                                 OpBuilder* b) {
  Type elementType = getElementTypeOrSelf(adaptor.getOperand().getType());
...
inline Value mapStableHloOpToStdScalarOp<stablehlo::LogisticOp>(
  ...
  Value expNegX = mapStableHloOpToStdScalarOp<stablehlo::ExpOp>(loc, resultTypes,
                                                      resultTypes, {{negX}}, b);
  ...
  return mapStableHloOpToStdScalarOp<stablehlo::DivOp>(loc, resultTypes, resultTypes,
                                             {{one, oneAddExprNegX}}, b);
}

template <>
inline Value mapStableHloOpToStdScalarOp<stablehlo::PowOp>(Location loc,
                                                 ArrayRef<Type> resultTypes,
                                                 ArrayRef<Type> argTypes,
                                                 stablehlo::PowOp::Adaptor adaptor,
                                                 OpBuilder* b) {
  ...
  if (resultType.isa<ComplexType, FloatType>()) {
    return MapStableHloOpToScalarOpImpl<IsFloatType, math::PowFOp, IsComplexType,
...
template <>
inline Value mapStableHloOpToStdScalarOp<stablehlo::SignOp>(Location loc,
                                                  ArrayRef<Type> resultTypes,
                                                  ArrayRef<Type> /*argTypes*/,
                                                  stablehlo::SignOp::Adaptor adaptor,
                                                  OpBuilder* b) {
...
struct StableHloOpToStdScalarOp {
  ...
  template <typename StableHloOpTy>
  static Value mapOp(StableHloOpTy op, ArrayRef<Type> resultTypes, ValueRange args,
                     OpBuilder* b) {
  ...
  static Value mapOpWithArgTypes(stablehlo::ConvertOp op, ArrayRef<Type> resultTypes,
                                 ArrayRef<Type> argTypes, ValueRange args,
                                 OpBuilder* b) {
  ...
  static Value mapOpOfType(Location loc, ArrayRef<Type> resultTypes,
    ...
    return impl::mapStableHloOpToStdScalarOp<StableHloOpTy>(loc, resultTypes, argTypes,
                                                  adaptor, b);
  }
};
...
#endif  // MLIR_HLO_StableHLO_TRANSFORMS_MAP_STABLEHLO_TO_SCALAR_OP_H
